...
События сервиса регистрируются в журнале работы. Расположение журнала, а также параметры журналирования настраиваются в конфигурационном файле [.ph .filepath]`[Каталог установки модуля {pl}]\Platform\5.5\FullTextService.Service.exe.config` в секции configuration/nlog.
...
Путь к файлу журнала указан в атрибуте `fileName` элемента `target[xsi:type="File"]`. Файл журнала по умолчанию – [.ph .filepath]`[Каталог установки модуля {pl}]\Platform\5.5\logs\\{Дата}.log`.
