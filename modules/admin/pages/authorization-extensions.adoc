...
Использование расширения ЕСИА доступно, начиная с версии модуля {wc} 5.5.6478.56 и версии модуля {pl} 5.5.7821.0.
...
<.> Отпечаток сертификата для работы с ЕСИА.
<.> Пароль сертификата для работы с ЕСИА. Необходимо указывать, если установлен сертификат с паролем. Если параметр не указан, в доступе к ЕСИА со страницы логина {wc}а не будет.
