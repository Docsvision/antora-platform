...
Начиная с модуля {pl} версии {pl-cu5}, настройка модуля может выполняться в конфигурационном файле модуля, `appsettings.json` или в интерфейсе {of-mc}, не требующем работы с файлами формата .json. Подробнее о настройке модуля в {of-mc} см. <<mancons,ниже>>.

Все настройки модуля хранятся . Конфигурационный файл может быть изменён в любом текстовом редакторе, например `nano`.
...
----
{
  "DocsVision": {
    ...
    "ServiceId": "d2f23ed0-9fea-8486-9780-b8192d334d28", <.>
    "SettingsService": {
      "ConnectionString": "http://storage-server.com:5200/api", <.>
      "ApiKey": "dvappserverapikey" <.>
  },
  "Groups": {
