...
* Если база данных не является единственной на сервере {dv}, xref:dataBaseDetach.adoc[отключите] БД от сервера {dv}.
...
После xref:dataBaseDetach.adoc[отключения] БД от одного сервера, её можно xref:dataBaseConnect.adoc[подключить] к другому.
****
