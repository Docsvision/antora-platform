...
****
Начиная с модуля {pl} версии {pl-cu5}, поддерживается два варианта включения отладки ролевой модели:

* При помощи конфигурационного файла `appsettings.json` модуля {pl}, подробнее см. ниже
* Из интерфейса {of-mc} без необходимости работы с форматом `.json`: "xref:dev@mgmtconsole:servers:db-config.adoc#role-debug[Включение отладки ролевой модели]"
****
