:with-console: no
:with-config: no
:do-what: варианта подключения БД
...
* Используйте существующую БД или создайте новую при помощи {of-mc}, см. xref:dev@mgmtconsole:servers:db-master.adoc[], без необходимости работы с форматом `.json`.
* Укажите строку подключения к БД в конфигурационном файле:
+
[source,json]
----
{
...
+
WARNING: {cns} устанавливается вместе с пакетом утилит {rk}.
+
. Откройте конфигурационный файл программы {cns} по адресу `C:\Program Files (x86)\Docsvision\Platform\Tools\appsettings.json`:
+
[source,json]
----
{
