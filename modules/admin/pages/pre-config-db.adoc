...
.. Для работы xref:dev@platform:ROOT:requirements-server-account.adoc[сервера {dv}], сервиса xref:dev@platform:ROOT:requirements-full-text-account.adoc[полнотекстового поиска] и xref:dev@platform:ROOT:requirements-file-service-account.adoc[файлового сервиса] может использоваться одна учетная запись.
+
