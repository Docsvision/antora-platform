...
[NOTE]
====
PgAgent отсутствует в репозиториях SberLinux и Pangolin.

Служба подходящей версии может быть получена с `postgresql.org`, в случае с {l-3} и {l-4} это https://download.postgresql.org/pub/repos/yum/15/redhat/rhel-9-x86_64/[Red Hat Enterprise Linux 9 и {pgsql} 15]. Установите службу вместе с `libpq` из штатного репозитория. После установки необходимо скопировать расширение из `/usr/pgsql-15/share/extension/` в `/usr/pangolin-6.4/share/extension/`.
