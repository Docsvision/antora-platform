...
.. Для работы xref:ROOT:requirementsServerAccount.adoc[сервера {dv}], сервиса xref:ROOT:requirementsFullTextAccount.adoc[полнотекстового поиска] и xref:ROOT:requirementsFileServiceAccount.adoc[файлового сервиса].
+
