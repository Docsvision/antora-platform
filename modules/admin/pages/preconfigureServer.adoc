...
.. Для работы xref:platform:ROOT:requirementsServerAccount.adoc[сервера {dv}], сервиса xref:platform:ROOT:requirementsFullTextAccount.adoc[полнотекстового поиска] и xref:platform:ROOT:requirementsFileServiceAccount.adoc[файлового сервиса].
+
