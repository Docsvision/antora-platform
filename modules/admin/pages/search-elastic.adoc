...
. Перезапустите {es}.
+
От имени администратора выполните в консоли `"C:\Program Files\Elastic\Elasticsearch\bin\{es}-plugin.bat" install \file:///%%path-to-ingest-attachment-{es-d}.zip%%`.
+
...
.. Откройте конфигурационный файл {es}: `C:\ProgramData\Elastic\{es}\config\Elasticsearch.yml`.
