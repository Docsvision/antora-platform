...
`C:\ProgramData\Elastic\{es}\config`.
...
+
.Путь до папки:
- `C:\ProgramData\Elastic\{es}\config\hunspell\ru_RU` для файлов https://cgit.freedesktop.org/libreoffice/dictionaries/plain/ru_RU/ru_RU.dic[ru_RU.dic] и https://cgit.freedesktop.org/libreoffice/dictionaries/plain/ru_RU/ru_RU.aff[ru_RU.aff]
- `C:\ProgramData\Elastic\{es}\config\hunspell\en_US` для файлов https://cgit.freedesktop.org/libreoffice/dictionaries/plain/en/en_US.dic[en_US.dic] и https://cgit.freedesktop.org/libreoffice/dictionaries/plain/en/en_US.aff[en_US.aff].
+
[WARNING]
====
...
. Перезапустите {es}.
+
От имени администратора выполните в консоли `"C:\Program Files\Elastic\{es}\bin\{es}-plugin.bat" install \file:///%%path-to-ingest-attachment-{es-d}.zip%%`.
+
...
.. Откройте конфигурационный файл {es}: `C:\ProgramData\Elastic\{es}\config\{es}.yml`.
...
----
+
Журнал работы системы располагается в папке `C:\ProgramData\Elastic\{es}\logs`.
+
[NOTE]
====
