...
+
. Установите компонент "Ingest Attachment Processor" (позволяет индексировать файлы карточек) для Elasticsearch версии {es-v}.
+
Компонент "Ingest Attachment Processor" можно установить из диалога установки Elasticsearch, поставив флаг:
+
