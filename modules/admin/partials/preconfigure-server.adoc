...
. Отключите контроль учётных записей (UAC) следующим образом:
...
. Убедитесь, что у учётной записи, под которой исполняется процесс IIS (по умолчанию учётная запись _Network Service_), есть права на чтение следующих папок:
...
[#users]
. Создайте в {pgsql} пользователей и назначьте для пользователей соответствующие права:
.. Для работы
ifdef::platform[]
...
+
Может использоваться одна учётная запись.
+
