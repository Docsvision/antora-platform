...
Начиная с модуля {pl} версии {pl-cu5}, отключеить и подключить БД можно двумя способами:

* С помощью программы {cns}, см. подробнее в разделах "xref:db-detach.adoc[Отключение]" и "xref:db-connect.adoc[Подключение]".
* Из интерфейса {of-mc}: см. подробнее в разделах xref:dev@mgmtconsole:servers:db-detach.adoc[Отключение] и xref:dev@mgmtconsole:servers:db-connect.adoc[Подключение].
...
+
** "xref:db-satellite.adoc[]" для программы {cns}.
** "xref:dev@mgmtconsole:servers:db-satellite.adoc[]" для {of-mc}.
====
