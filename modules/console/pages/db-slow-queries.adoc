...
- Ожиданияв {pgsql} -- https://simply.name/ru/pg-stat-wait.html[simply.name].
...
Самый простой способ выявить медленные запросы в {pgsql} -- посмотреть журнал медленных запросов. По умолчанию журнал медленных запросов не ведётся, но его можно включить. Есть два способа включения: глобально -- путём редактирования `{pgsql}.conf` и локально -- для определённого пользователя или БД.

.Чтобы включить журнал медленных запросов глобально, в конфигурационном файле `{pgsql}.conf` измените следующую строчку:
----
...
.Чтобы активировать *pg_stat_statements*, добавьте в конфигурационный файл `{pgsql}.conf` следующую строку и перезапустите сервер БД:
[source,pgsql]
----
...
Подробнее ознакомиться с методами выявления медленных запросов в {pgsql} можно в https://www.cybertec-{pgsql}.com/en/3-ways-to-detect-slow-queries-in-{pgsql}/[статье Hans-Jürgen Schönig] (анг.), с переводом статьи можно ознакомится на https://habr.com/ru/post/488968/[habr.com]. Подробнее о выявлении медленных запросов при помощи *pg_stat_statements* в https://www.cybertec-{pgsql}.com/en/pg_stat_statements-the-way-i-like-it/[другой статье] того же автора.