...
+
****
Начиная с модуля {pl} версии {pl-cu5}, поддерживается обновление БД двумя способами:

* С помощью программы {cns}, см. подробнее "xref:db-update.adoc[]".
* Из интерфейса {of-mc}: "xref:dev@mgmtconsole:servers:db-update.adoc[]".
****
