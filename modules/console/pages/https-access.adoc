...
При необходимости можно настроить доступ к службе Сервера {dv} по протоколу HTTPS, для этого потребуется выполнить настройки, описанные далее.
...
. Перезапустите службы Сервера {dv} и {wc}а.
+
