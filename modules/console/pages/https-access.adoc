...
. Укажите адрес Storage Server с `https`
+
* (*Предпочтительный способ*) Откройте {mc} а странице "xref:dev@mgmtconsole:servers:appserver-access.adoc[]"
* Откройте {cns} на вкладке "xref:manage-access.adoc[]" в полях _Адрес сервера при подключении_ и _Адрес сайта при подключении через внешнюю сеть или по HTTP_.
+
