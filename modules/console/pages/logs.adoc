...
По умолчанию журнал сервиса сохраняется в папку `/var/log/docsvision/appserver/dvappserver.log` в Linux и `C:\ProgramData\docsvision\appserver\dvappserver.log` в Windows. Путь к журналу и уровень журналирования можно настроить в конфигурационном файле Сервера приложений (модуля {pl}), подробнее работа с конфигурационным файлом описана в разделе "xref:admin:config-platform.adoc[]"
...
[#search]
== Журнал сообщений сервиса полнотекстового индексирования
...
:module-service: полнотекстового индексирования
