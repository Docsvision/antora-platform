...
Процесс миграции с БД типа MS SQL Server на PosgreSQL состоит из нескольких этапов. Это позволяет лучше контролировать каждый шаг миграции, а также позволяет продолжить миграцию с места остановки, например, при возникновении ошибки. Т.к. {pgsql} не поддерживает Сателлитные БД, миграция в {pgsql} подразумевает перенос архивных данных и журналов из соответствующих БД {mssql} (*_Archive* и *_Log*) в основную.
