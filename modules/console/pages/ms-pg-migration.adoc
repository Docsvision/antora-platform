...
Процесс миграции с БД типа MS SQL Server на PosgreSQL состоит из нескольких этапов. Это позволяет лучше контролировать каждый шаг миграции, а также позволяет продолжить миграцию с места остановки, например, при возникновении ошибки.
...
Утилита может использоваться в режиме тестирования соединения, например, для проверки возможности установления соединения с базой-источником {mssql} и базой-приемником {pgsql}. Примеры использования приведены ниже:
...
/ms: MsSql database connection string (Default is "")
/pg: PgSql database connection string (Default is "")
/p: Export/Import folder path (Default is "")
/install[+|-] Install export objects
/clone[+|-] Clone database structure
/export[+|-] Export tables to csv files
/l: Limits number of tables to export per call (Default is "0")
/import[+|-] Import tables from csv files
/test[+|-] Tests connection to database(s)
/f[+|-] Normalize file
/in: Input file path (Default is "")
/out: Output file path (Default is "")
/infolder: Input folder with csv files (Default is "")
/outfolder: Output folder for converted csv files (Default is "")
/s[+|-] Split file
/m: Maximum number of rows per file (for split) (Default is
"2000")
/help Show usage.
----
====
...
. Оцените размер таблицы `dvsys_binires`.
. Подготовьте файловое хранилище необходимого объема, больше, чем текущий размер таблицы `dvsys_binires`.
...
+
. Дождитесь завершения обработки созданной очереди *{fss}*.
...
----
+
. Перенесите `.csv` файлы на сервер {pgsql}. +
Для импорта `.csv` файлов в базу {pgsql} их необходимо разместить на диск той машины, где развернут {pgsql}.
+
...
----
+
. Установите заново все серверные расширения {dv}.
