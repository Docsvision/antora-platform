...
В данном разделе приведена инструкция по установке {es} на Windows и Linux. Перед установкой ознакомьтесь с требованиями системы {es} в разделе xref:ROOT:requirements-hardware.adoc#elastic[по ссылке].
...
ОС Windows::
----
"C:\Program Files\Elastic\Elasticsearch\bin\Elasticsearch-plugin.bat" install \file:///%%path-to-ingest-attachment-{es-d}.zip%% <.>
----
...
+
. При установке на Linux убедитесь, что значение `vm.max_map_count` больше или равно `262144`. При установке на Windows пункт можно проигнорировать.
+
...
----
+
. Перед запуском нужно убедиться, что `vm.max_map_count` больше или равно `262144`. Задать значение можно следующим образом:
+
[source]
----
echo "vm.max_map_count=262144" | sudo tee /etc/sysctl.d/elasticsearch.conf
