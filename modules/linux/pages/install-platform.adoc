...
Данный раздел описывает установку модуля {pl} для ОС Linux. Все описанные ниже команды необходимо выполнять после завершения xref:pre-config-server.adoc[предварительной настройки сервера].
